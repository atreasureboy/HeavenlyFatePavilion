...
### 十七次边界深化：资源恢复产生候选任命，不产生自动圣旨

来源：2026-08-14 CodeTMux 在动态 Admission 之上实现中央持久候旨队列。

资源不足时直接丢弃任务会迫使仙人反复记忆和重试，但资源恢复后由节点自动启动队首任务又会把“容量事实”错误提升为“中央意志”。队列应保存尚未任命的候选差事，并把容量、优先级和等待公平性变成可解释的排序依据；真正启动仍是一项新的主权动作。

候选不变量如下：

1. 候旨记录与 Runtime 任命是两种事实。`QUEUED` 表示仍有阻碍，`READY` 表示当前具备任命条件，只有显式 `DISPATCH` 才能创建 Runtime；
2. 节点在线、协议版本、Provider 可用性、Runtime Key、资源准入和项目所有权都属于 READY 的新鲜前置条件；派发时执行节点必须再次核对，不能消费过期快照；
3. 队列属于中央持久状态，地方节点不拥有挑选下一任务的权力。节点只上报容量变化，不能在空出槽位后自行领取工作；
4. 排序使用显式基础优先级并叠加等待老化，防止后台任务永久饥饿；排序只决定中央首先审查谁，不等于自动授权；
5. 人工紧急指令可以提高候旨优先级，但观察、取消、停止和仙人直达不进入普通公平队列，它们始终属于高于容量调度的控制面；
6. 候旨请求必须有稳定幂等键。传输重试、页面刷新和 OVO 重复调用都只能返回同一任命记录；若相同幂等键绑定不同 Host、Runtime、Provider 或目录则拒绝；
7. 中央在 `dispatching` 中重启时不得根据中间状态自动补发。该记录退回待核对，并结合 Runtime Registry、连接代次与节点现场判断是否已经产生实际任命；
8. 队列只保存启动所需的白名单字段，不保存模型密钥。已派发、取消和失败记录保留为有限历史，不能为了队列简洁删除已发生的任命事实；
9. 真实模型额度、租约与跨主机成本未来可成为 Blocker 或排序信号，但模型建议不能直接篡改队列事实和优先级来源。

因此候旨队列的治理含义是：**“现在有资源”只是可以下旨的情报，不是已经下旨；调度可以辅助皇帝安排次序，不能替皇帝任命臣子。**

### 十八次验证反馈：预检与执行闸门不一致会制造假情报

来源：2026-08-14 对 CodeTMux 动态 Admission 与候旨链路进行反向模拟；本节是实现验证反馈，不代表问题已经修复。

模拟证明“READY 不自动任命”和 Runtime Pool 最终 fail-closed 已真实成立，但也暴露出一个治理层常见陷阱：中央候旨预检只看 Provider Admission 与同 Key 活跃状态，地方最终执行还检查 Host 总容量和 cwd 所有权。于是中央会收到“可以任命”的假情报，真正下旨后才被地方拒绝。虽然没有造成越权写入，但反复错误决策会削弱无人值守可靠性。

新增验证约束如下：

1. 预检、排序、Center 展示和最终执行必须消费同一版本的准入决策模型；允许最终检查更“新鲜”，不允许预检在语义上少一类硬约束；
2. `READY` 必须携带判定版本、证据时间和完整 Blocker 集合。最终因现场变化拒绝是正常竞态，因预检从未检查 Host 容量/cwd 则是实现错误；
3. `dispatching` 遇到中央重启、连接更代或 ACK 超时时应进入 `execution-unknown / needs-reconciliation`，不能退回普通候旨。只有核对 Runtime Registry、节点当前实例和稳定任命身份后才能重派；
4. 候旨 `appointmentId` 应成为地方启动的业务幂等身份，如同 Turn ID；网络请求 ID 只能关联一次传输，不能证明是否已经任命；
5. CPU 持续压力必须由独立时间采样形成，而不是靠用户反复点击启动累计。主机心跳应更新压力窗口和候旨状态，但只能产生 READY 事件，不能自动任命；
6. 公平老化不得越过仙人明确标记的 urgent 权限带。防饥饿可以提升同一治理等级内的顺序，不能用等待时间覆盖人类紧急主权；
7. 辅助治理账本损坏不得拖垮中央启动。应从备份恢复、隔离损坏文件并进入 degraded；“队列不可用”不能升级为“整个 AgentOS 不可用”；
8. 只读状态查询中同步重写完整队列会把治理模块重新变成主线程卡顿源。状态转移应异步、增量、有界，Snapshot 本身保持无副作用；
9. 真实模型额度尚未接入时，Provider 并发只能称本地策略上限，不能包装成上游额度事实。

这次验证把候旨原则进一步收紧为：**最终闸门防止做错事，统一预检防止中央不断想错事；两者缺一，系统都不能称为可靠自治。**

### 十九次工程回证：统一判定、业务幂等与未知执行协调已形成可运行闭环

来源：2026-08-14 CodeTMux 对第十八次验证发现执行修复，并完成本机、模拟远端、Windows CPU 语义与持久化损坏故障注入。状态：`Draft / Engineering Feedback`，用于约束后续正式运行体系设计，不把单项目测试直接提升为企业级基线。

工程回证表明，第十八次提出的原则可以落为一条不依赖模型自觉的确定性链路：

```text
Host Heartbeat / 定时资源采样
  → 版本化 RuntimeStartDecision
  → 候旨预检（预检可任命）
  → 中央显式 DISPATCH
  → Relay v9 + stable appointmentId
  → 地方 Runtime 最终新鲜复检
  → ACK / Runtime 现场回流
  → dispatched 或 needs-reconciliation
```

本轮获得的可复用结论如下：

1. **同一判定模型不等于复用同一份旧快照。** Queue 与执行节点共享约束类别、Blocker 代码和目录身份算法；执行节点仍必须重新采样。预检与最终结果因现场变化不同是正常竞态，因规则集合不同而长期冲突才是架构错误；
2. **任命身份必须穿过传输边界。** `requestId` 关联一次 Socket 请求，`appointmentId` 标识一次业务任命。地方对相同任命的新 requestId 返回原事实，不能再次创建 Runtime；Runtime Snapshot 也保留任命身份供中央重启后核对；
3. **超时不是失败，更不是未执行。** `dispatching` 重启、ACK 超时和连接中断进入 `needs-reconciliation`。中央只在新鲜现场出现相同任命身份时自动采纳；否则保持未知，直到人类或可靠探针确认未启动或决定取消；
4. **持续压力是时间事实，不是调用次数。** CPU 高压窗口由心跳/定时采样推进，重复读取 Snapshot 不能加速窗口；Windows 以系统 CPU tick 差值补足无 load average 的平台差异。资源恢复只发布候选事件，不触发自动任命；
5. **人工紧急带不可被公平老化穿透。** 等待老化解决普通候选饥饿，但非 urgent 的有效优先级必须封顶在 urgent 以下；公平性不得改写人类明确紧急权；
6. **辅助账本必须可降级启动。** 候旨文件采用校验、备份、损坏隔离与恢复后主文件重建；主备同时损坏时队列降级为空并暴露健康状态，而不是阻断整个中央控制面；
7. **恢复路径本身必须二次复测。** 仅证明“第一次能从备份读出”不够，还要证明恢复后再次重启不会丢失；仅证明“CPU 首次高压不拒绝”也不够，还要验证压力状态不会在下一次高压样本被错误清零；
8. **界面应表达证据等级。** `READY` 对用户呈现为“预检可任命”，未知执行提供“核对现场 / 确认未启动 / 取消未知任命”，避免把内部状态名包装成确定事实。

当前仍保留三项非阻断边界：Provider 并发是本地主机策略而非上游真实额度；小型 JSON 队列适合个人控制面但不等价于企业级增量 Ledger；Daemon 崩溃后对脱离父进程的 Runtime 进行操作系统级再发现仍需单独设计。

因此本轮不是证明“整个自治系统已经完成”，而是证明一个更窄且重要的事实：**候旨到任命的控制链已经能在容量变化、传输重试、中央重启和辅助状态损坏下保持主权语义，不再依赖模型猜测执行结果。**
