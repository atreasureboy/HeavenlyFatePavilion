...
## 十四次边界深化：地方能力发布不是即时授予（2026-08-14）

地方节点可以比中央更了解本机已有的 Skills、MCP 和开发工具，但“知道某项能力存在”与“允许某个 Runtime 使用它”必须是两个动作。节点只发布规范化描述符、版本、适用 Provider、所需能力和内容哈希；正文与执行入口留在当地。中央基于 `节点 + 资产 + 哈希` 审核，具体 Mandate 启动时再选择挂载，地方则按当前文件和当前能力二次核对。

这形成新的权力不变量：

1. 发布权属于地方，授权权属于中央，使用权只存在于一次明确的 Runtime 委任中；
2. 地方更新正文、命令或版本即改变哈希，前朝批准自动失效，不能把“同名能力”视为同一能力；
3. Skill 正文无需进入中央总账，中央掌握身份和批准证据即可，执行时在目标节点本地注入；
4. MCP 描述符不得夹带环境变量和凭据；批准一个工具入口不等于批准它扩大主权 Capability；
5. 旧协议若不能理解挂载清单必须拒绝连接，不能静默启动一个缺少获批能力的 Runtime 后谎报成功；
6. 皇帝可以允许臣子使用地方知识和工具，但地方声明、中央批准、Runtime 实际挂载三者都必须可区分、可撤销、可审计。

这一机制把“地方因地制宜”纳入中央秩序：它避免中央复制每台主机的全部知识，也避免地方以动态发现为名自行扩权。

### 十五次边界深化：主机容量属于准入治理，运行实例必须拥有独立失效域

来源：2026-08-14 CodeTMux 完成一 Relay 多 Runtime 首阶段工程闭合。

单一 Host Control Plane 可以承载多个项目的成熟 Coding Runtime，但“共享一台主机”不能被实现成“共享一个会话、队列和生命周期”。扩展容量时必须先建立 Runtime 级身份与隔离，再讨论调度密度；否则任何一次取消、输入缺口、项目冲突或 Provider 崩溃都会污染同主机其它任务。

1. Host 是容量与连接边界，Runtime 是认知、项目和失效边界；每个 Runtime 必须具有稳定 `runtimeKey`、瞬时 `runtimeInstanceId`、独立 Session、cwd、Turn Ledger、输入序列、Seal 状态与能力挂载；
2. Admission 是中央与确定性节点共同执行的硬规则，不由模型自行决定。准入至少检查 Host 并发上限、Runtime Key 唯一性、Provider 当前可用性和项目目录所有权；
3. 同一项目工作树默认只能有一个可写 Runtime。需要并行开发时应先建立不同 Worktree/目录，再分别准入，不能只换 Runtime Key 掩盖文件冲突；
4. 同 Runtime 的命令严格 FIFO，不同 Runtime 的命令允许并发。Host 全局队列会让一个长任务阻塞所有项目，也会把无关输入序列错误地耦合；
5. 容量满载只拒绝新的 `start`，不得剥夺现有 Runtime 的 `status / report / cancel / stop`，因为观察和收回能力属于最低生存内核；
6. 停止、取消、Seal、Session 恢复和 Provider 故障必须限定在目标 Runtime。一个项目退场不能终止或改写同 Host 其它项目的期望状态；
7. 中央 Registry 的键必须从单一 Host 扩展为 `Host + RuntimeKey`，并继续把期望状态与新鲜观察分开；Host 在线不能证明其中每个 Runtime 仍在履职；
8. Center 同时显示 Host 总容量、活跃槽位和每个 Runtime 的协调状态，使仙人能够切换目标并独立下旨、核对和收回；
9. 第一阶段使用静态并发上限和 cwd 互斥。CPU、内存、模型额度、公平性与优先级属于后续动态 Admission，不得在没有观测证据时伪装成已经完成的资源治理；
10. 多 Runtime 承载不改变主权关系：地方 Pool 只执行中央准入与生命周期命令，Runtime 仍是可替换臣子，不能因共享主机或持有本地工具获得跨项目权力。

因此容量扩展的正确顺序是：**先把每个 Runtime 变成独立、可观察、可收回的对象，再允许一台 Host 同时承载多个项目；并行数不是能力本身，隔离后的可控并行才是。**

### 十六次边界深化：资源压力只能阻止新任命，不能切断中央收回权

来源：2026-08-14 CodeTMux 将地方多 Runtime 从静态槽位推进到内存、持续 CPU 压力与 Provider 并发的动态准入。

动态容量治理不能简单实现成“主机忙时整个节点不可用”。若资源高压同时关闭状态、报告、取消和停止，中央恰好会在最需要降载时失去收回臣子的能力。主机遥测也不是绝对事实：Windows 与 Linux 可提供的负载指标不同，采样器故障不能被解释成资源充足，更不能被解释成所有控制动作都应失败。

候选不变量如下：

1. Admission 只裁决新的 Runtime 任命，不裁决既有 Runtime 的观察、奏报、取消、输入核对与停止；后者属于节点的最低生存控制面；
2. 实际启动前必须在执行节点读取新鲜资源事实并再次准入。中央 Center 的容量快照用于解释和预选，不是可跨时间使用的启动许可证；
3. 可用内存低于保留水位可立即拒绝新建；CPU 应采用负载平均或连续采样等持续压力证据，避免一次瞬时峰值造成抖动；
4. 总 Runtime 槽位、同 Provider 并发和项目目录所有权是不同约束，应分别给出机器可读原因，不能统一成模糊的“节点繁忙”；
5. Provider 配额必须按真实活跃 Runtime 计数，并允许中央配置；模型额度、并发租约或上游限流事实尚未接入时，不得把静态猜测包装成真实额度；
6. 遥测不可用应进入 `degraded` 并明确缺失的探针。系统可以在保留静态容量与互斥检查后谨慎放行，但必须留下可观察告警；
7. 资源准入失败不得偷偷停止、迁移或替换现有 Runtime。迁移与抢占是独立主权决策，需要保存 Session、项目所有权与未完成 Turn 事实；
8. 后续公平排队和优先级只能决定“下一项新任命是谁”，不能改变仙人直达、中央核查和紧急收回的优先权。

因此动态 Admission 的准确边界是：**地方资源决定是否还能接新差事，中央主权决定如何管理已在任的臣子；主机越忙，收回与观察通道越不能被关掉。**
