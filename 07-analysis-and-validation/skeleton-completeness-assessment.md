# 天机阁整体架构骨架完整性审计

状态：Draft

审计日期：2026-07-24

## 一、结论

当前完成的是 `Skeleton v0.1`，可以称为“一级骨架基线”，不能称为“整体骨架已经完善”。

已经具备：

- 五个视觉分组；
- 十个一级逻辑面；
- 第一批二级能力域；
- 三条核心闭环；
- 七个横切维度；
- Agent / Tool 默认分工方向；
- 基础设施的共同承载位置；
- 第一批文字架构图。

仍然缺少足以证明骨架闭合的接口、对象、状态、异常和映射。

## 二、骨架完善的判定标准

整体骨架只有同时通过以下七道门，才能进入“结构完善、局部待深化”状态。

| 完整性门 | 判定问题 | 当前状态 |
|---|---|---|
| G1 顶层覆盖 | 所有核心问题能否归入稳定一级面 | 基本通过 |
| G2 二级覆盖 | 每个一级面是否拥有完整且类型一致的二级能力树 | 部分通过 |
| G3 接口闭合 | 每个面是否明确输入、输出、调用方、被调用方和契约 | 未通过 |
| G4 对象闭合 | 核心对象是否拥有唯一语义、所有者和状态层级 | 未通过 |
| G5 状态闭合 | 任务、事实、研判、产品、告警和版本是否有生命周期 | 未通过 |
| G6 横切闭合 | 多租户、权限、证据、预算、质量、审计如何落到每个面 | 未通过 |
| G7 场景闭合 | 正常、退回、冲突、失败和演进场景能否贯穿全骨架 | 未通过 |

## 三、当前最重要的结构缺口

### 3.1 缺少系统上下文边界

尚未完整画出：

- 仙人、租户、平台运营者和外部协作者；
- 外部数据源、模型提供者、消息渠道和企业系统；
- 天机阁系统边界内外的责任；
- 皇帝是租户级主体还是平台共享主体的实例关系。

### 3.2 缺少核心对象主干

需要建立贯穿十面的对象链：

```text
Intent
→ Mission
→ Plan
→ Run / Task
→ Source / Document / Snapshot
→ Evidence / Claim / Fact
→ Entity / Relationship / Event / Change
→ Assessment / Forecast / Risk
→ Intelligence Product
→ Feedback / Evolution Evidence
```

如果对象主干不确定，各面只是名称集合，无法形成真正契约。

### 3.3 缺少十面的输入输出契约

目前只定义了职责，没有逐面定义：

- 接受什么输入；
- 产生什么输出；
- 拥有什么持久状态；
- 调用谁；
- 谁可以拒绝它；
- 失败后向哪里反馈。

### 3.4 公共官署尚未完成职责映射

通政司、国史馆、文渊阁、钦天监、大理寺、都察院、翰林院和枢密院只有候选位置，尚未按真实职责拆分到十个一级面。部分官署可能跨面，需要区分叙事机构与技术组件。

### 3.5 六部只有一级领域标签

还缺少统一的“部级能力模板”：

```text
领域使命
领域本体
领域 Agent
领域 Tool / Skill
领域来源
领域数据产品
领域质量标准
与共享主干的扩展点
```

没有模板，六部后续会以六种不同方式膨胀。

### 3.6 缺少状态机和异常骨架

至少需要建立：

- Mission / Run / Task 生命周期；
- Claim / Fact / Disputed Claim 生命周期；
- Assessment / Forecast 校准生命周期；
- Risk / Alert 生命周期；
- Capability Version 生命周期；
- 退回、封驳、冲突、超预算、无证据和模型退化路径。

### 3.7 横切维度仍只是标签

“多租户、权限、证据、预算、质量、审计、生命周期”已经被列出，但尚未形成：

- 集中控制点；
- 全链路上下文字段；
- 每个一级面的责任；
- 不变量；
- 失败后果；
- 验证场景。

### 3.8 文字图集仍缺关键视图

当前已有总览、闭环、十面关系和朝廷图，仍需：

- 系统上下文图；
- 核心对象流转图；
- 控制流与数据流分离图；
- 十面输入输出契约图；
- 多租户横切图；
- 公共官署归属图；
- 正常与失败场景时序图；
- 能力演进作用全域图。

## 四、建议完成顺序

```text
第一步：系统上下文
    ↓
第二步：核心对象主干
    ↓
第三步：十面输入输出契约
    ↓
第四步：二级能力树补全
    ↓
第五步：三省、六部、公共官署映射
    ↓
第六步：核心状态机与异常路径
    ↓
第七步：横切维度落位
    ↓
第八步：端到端场景验证
    ↓
整体骨架完善
```

这个顺序仍然是从整体到局部。它先补骨架的连接处，再深入任何单个部门或技术专题。

## 五、当前阶段命名

```text
已经完成：一级骨架定型
正在进行：整体骨架闭合
尚未开始：局部专题深度设计
```

因此，当前不应宣布“天机阁整体架构骨架已经完善”，只能宣布“一级骨架已经建立，整体骨架闭合工作开始”。

## 六、关联文档

- [整体架构骨架](../03-current-architecture/overall-architecture-skeleton.md)
- [文字架构图集](../03-current-architecture/text-diagrams/README.md)
- [阶段 1：整体架构骨架访谈](../02-architecture-iterations/v0001-concept-baseline/phase-01-overall-skeleton-interview.md)
